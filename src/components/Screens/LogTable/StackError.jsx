...
export default function StackError() {
  ...
  // ! -----------------------------------------------------------need to add array toggle
  const [displayLowerInfo, setDisplayLowerInfo] = useState({
    activeItme: null,
  });

  const [displayLowerInfoStackError, setDisplayLowerInfoStackError] =
    useState(true);

  const [displayLowerInfoWithOutStack, setDisplayLowerInfoWithOutStack] =
    useState(false);
  ...
  const ShowInnerInfo = (index) => {
    if (displayLowerInfo.activeItme == null) {
      setDisplayLowerInfo({ ...displayLowerInfo, activeItme: index });
    }
    if (displayLowerInfo.activeItme >= 1 || displayLowerInfo.activeItme == 0) {
      setDisplayLowerInfo({ ...displayLowerInfo, activeItme: null });
    }
  };
  ...
  const ShowInnerInfoNew = () => {
    if (!displayLowerInfoWithOutStack) {
      setDisplayLowerInfoWithOutStack(true);
    } else {
      setDisplayLowerInfoWithOutStack(false);
    }
  };
      ...
      <Container>
        <div style={{ marginTop: "6%", width: "84%", float: "right" }}>
          <Row className="mt-5 d-flex align-center">
            ...
            <Col
              xl={7}
              className={`${Style.versonGraph} m-2 p-4 d-flex align-items-center`}
            >
              {completeStackMultplenewArrayNew ? (
                <VersonGraph
                  logMsg={completeStackMultplenewArrayNew}
                  code={code}
                />
              ) : (
                ...
                <Row className="mt-4 d-flex">
                  <div className={Style.innerdiv}>
                    <div style={{ display: "flex" }}>
                      <p onClick={ShowInnerInfoStackError}>
                        Stack Errors
                    ...
                    <Col xl={12}>
                      <div>
                        <div className={`${Style.NoStackErrorDiv}`}>
                          <p onClick={ShowInnerInfoNew}>
                            {completeStack}
                            <span style={{ float: "right" }}>
                              <FontAwesomeIcon icon={faArrowDown} />
                            </span>
                          </p>
                          {displayLowerInfoWithOutStack ? (
                            <p
                              style={{
                                color: "#707070",
                                fontSize: ".9rem",
                        ...
                        <Col xl={12} className="mt-2">
                          <div>
                            <div className={`${Style.stackErrorDiv}`}>
                              ...
                              {displayLowerInfo.activeItme === index ? (
                                <p
                                  style={{
                                    color: "#707070",
                                    fontSize: ".9rem",
